...
class PteSyncMasterPatients extends Command
{
    protected $signature = 'pte:sync-master-patients
    ...
    protected $description = 'Pull patient master data from PtEverywhere API and store locally';

    public function handle(PtEverywhereService $api, PtePullHistoryLogger $historyLogger): int
    {
        $this->info('Fetching master patients from PtEverywhere...');
        $history = null;
        ...
        try {
            ...
            $history = $historyLogger->start($this->getName() ?? 'pte:sync-master-patients', [
                'source_key' => 'master_patients',
            ...
            [$fetched, $created, $updated] = $this->syncPatients($api, $pageSize, $limit);
     ...
     */
    private function syncPatients(PtEverywhereService $api, int $pageSize, ?int $limit): array
    {
        $fetched = 0;
        $created = 0;
        $updated = 0;
        ...
        do {
            ...
            foreach ($docs as $row) {
                ...
                $record = PteMasterPatient::updateOrCreate(
                    ['row_key' => $rowKey],
                    [
                        'pte_patient_id' => $this->toStringOrNull($row['_id'] ?? $row['id'] ?? null),
                        'patient_code' => $this->toStringOrNull($row['patientCode'] ?? $row['code'] ?? null),
                        ...
                        'date_of_birth' => $this->toStringOrNull($row['dateOfBirth'] ?? $row['dob'] ?? null),
                        'gender' => $this->toStringOrNull($row['gender'] ?? null),
                        'address' => $this->toStringOrNull($row['address'] ?? $row['address1'] ?? null),
                        ...
                        'status' => $this->toStringOrNull($row['status'] ?? $row['patientStatus'] ?? null),
    ...
    {
        $patientId = trim((string) ($row['_id'] ?? $row['id'] ?? ''));
        if ($patientId !== '') {
        ...
        $parts = [
            ...
            trim((string) ($row['patientCode'] ?? $row['code'] ?? '')),
